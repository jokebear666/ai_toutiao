...
- **[arXiv260101] Time-Aware Adaptive Side Information Fusion for Sequential Recommendation**
  - **tags:** [ai], [sequential recommendation], [side information fusion, temporal dynamics, adaptive filtering]
  - **authors:** Jie Luo, Wenyu Zhang, Xinming Zhang, Yuan Fang
  - **institution:** University of Science and Technology of China, Singapore Management University
  - **link:** https://arxiv.org/pdf/2512.24246
  - **code:** https://github.com/jluo00/TASIF
  - **contributions:** 1. A plug-and-play time span partitioning mechanism to capture global temporal patterns. 2. An adaptive frequency filter that uses a learnable gate to denoise feature sequences. 3. An efficient adaptive side information fusion layer with a "guide-not-mix" architecture for deep interaction without mixing attributes into item embeddings.
  - **thumbnail:** https://pub-9ba4a5dae3bf4fc7b7d26411a74e92db.r2.dev/thumbnails/a11c0b345833a0a377b18e0217b0b567071b00be6a8e2b525f74cb65d753c493_w640_q70.webp
  - **Simple LLM Summary:** The paper proposes the TASIF framework to address challenges in sequential recommendation, including overlooking temporal dynamics, vulnerability to noise, and high computational cost. It introduces three components: time span partitioning, an adaptive frequency filter, and an efficient fusion layer. Experiments show TASIF outperforms state-of-the-art baselines while maintaining training efficiency.
  - **Mindmap:**

    ```mermaid
    graph TB
    A[Time-Aware Adaptive Side Information Fusion for Sequential Recommendation] --> B[核心问题/Problem]
    A --> C[主要方法/Method]
    A --> D[关键结果/Results]
    B --> B1[忽略细粒度时间动态/Ignore fine-grained temporal dynamics]
    B --> B2[序列噪声敏感/Vulnerable to sequence noise]
    B --> B3[融合架构计算昂贵/Computationally expensive fusion]
    C --> C1[时间跨度划分/Time span partitioning]
    C --> C2[自适应频率滤波/Adaptive frequency filter]
    C --> C3[高效自适应融合/Efficient adaptive fusion layer]
    D --> D1[性能显著提升/Significantly outperforms baselines]
    D --> D2[训练效率高/Maintains training efficiency]
    ```

- **[arXiv260101] RAGPart & RAGMask: Retrieval-Stage Defenses Against Corpus Poisoning in Retrieval-Augmented Generation**
  - **tags:** [mlsys], [rag (retrieval-augmented generation)], [corpus poisoning, dense retriever, retrieval-stage defense, document partitioning, token masking]
  - **authors:** Pankayaraj Pathmanathan, Michael-Andrei Panaitescu-Liess, Cho-Yu Jason Chiang, Furong Huang
  - **institution:** University of Maryland College Park, Capital One, Peraton Labs
  - **link:** https://arxiv.org/pdf/2512.24268
  - **contributions:** 1. Proposed RAGPart, a defense leveraging the training dynamics of dense retrievers and document partitioning to mitigate poisoned documents. 2. Proposed RAGMask, a defense that identifies suspicious tokens by analyzing similarity shifts under targeted token masking. 3. Introduced an interpretable attack to stress-test the proposed defenses and evaluated them across multiple benchmarks, poisoning strategies, and retrievers.
  - **thumbnail:** https://pub-9ba4a5dae3bf4fc7b7d26411a74e92db.r2.dev/thumbnails/eed99fde057eefd1431417b1d512eaa39807473cb3d4135031a0e9dae115d7a8_w640_q70.webp
  - **Simple LLM Summary:** This paper addresses the vulnerability of Retrieval-Augmented Generation (RAG) systems to corpus poisoning attacks, where adversaries inject malicious documents to manipulate outputs. It proposes two lightweight, retrieval-stage defenses, RAGPart and RAGMask, which operate directly on the retriever without modifying the generation model. The defenses were shown to consistently reduce attack success rates while maintaining utility under benign conditions, highlighting the potential of retrieval-stage approaches for robust RAG.
  - **Mindmap:**

    ```mermaid
    graph TB
        A[RAGPart & RAGMask: Retrieval-Stage Defenses Against Corpus Poisoning] --> B[核心问题/Problem: RAG系统易受语料库投毒攻击/RAG vulnerable to corpus poisoning attacks]
        A --> C[主要方法/Method: 提出两种检索阶段防御/Propose two retrieval-stage defenses]
        C --> D[RAGPart: 利用稠密检索器训练动态与文档分区/Leverages dense retriever training dynamics & document partitioning]
        C --> E[RAGMask: 基于目标令牌掩码的相似性偏移识别可疑令牌/Identifies suspicious tokens via similarity shifts under targeted token masking]
        A --> F[关键结果/Results: 在多种设置下持续降低攻击成功率，保持良性效用/Consistently reduces attack success rates across settings, preserves benign utility]
    ```
...
- **[arXiv260101] On the Factual Consistency of Text-based Explainable Recommendation Models**
  - **tags:** [nlp], [explainable recommendation], [factual consistency, large language models, natural language inference, statement-level evaluation, explainable recommendation models]
  - **authors:** Ben Kabongo, Vincent Guigue
  - **institution:** Sorbonne University, CNRS, ISIR, AgroParisTech
  - **link:** https://arxiv.org/pdf/2512.24366
  - **code:** https://github.com/BenKabongo25/factual_explainable_recommendation
  - **contributions:** 1. Introduced a comprehensive framework for evaluating the factual consistency of text-based explainable recommendation models. 2. Proposed a prompting-based pipeline using LLMs to extract atomic explanatory statements from reviews to construct a factual ground truth benchmark. 3. Designed statement-level alignment metrics combining LLM- and NLI-based approaches to assess factual consistency and relevance of generated explanations.
  - **thumbnail:** https://pub-9ba4a5dae3bf4fc7b7d26411a74e92db.r2.dev/thumbnails/d9c76b5ee4016c79058d120bfc7c050ef62a7ce8fc6f0623fe422173e38e2f33_w640_q70.webp
  - **Simple LLM Summary:** This paper addresses the problem of factual inconsistency in text-based explainable recommendation models. The authors propose a new evaluation framework that uses LLMs to extract factual statements from reviews as ground truth and combines LLM- and NLI-based metrics to assess explanation quality. Their experiments on six state-of-the-art models reveal a critical gap where models achieve high semantic similarity but alarmingly low factual consistency, highlighting the need for factuality-aware evaluation.
  - **Mindmap:**

    ```mermaid
    graph TB
        A[On the Factual Consistency of Text-based Explainable Recommendation Models] --> B(核心问题/Problem)
        A --> C(主要方法/Method)
        A --> D(关键结果/Results)
        B --> B1["解释的事实一致性是否可靠?<br/>Are explanations factually consistent?"]
        C --> C1["LLM提示管道构建事实基准<br/>LLM-prompting pipeline for factual ground truth"]
        C --> C2["语句级对齐评估指标<br/>Statement-level alignment metrics (LLM+NLI)"]
        D --> D1["高语义相似性<br/>High semantic similarity (BERTScore F1: 0.81-0.90)"]
        D --> D2["低事实一致性<br/>Low factual consistency (Precision: 4.38%-32.88%)"]
    ```

- **[arXiv260101] MEIC-DT: Memory-Efficient Incremental Clustering for Long-Text Coreference Resolution with Dual-Threshold Constraints**
  - **tags:** [nlp], [coreference resolution], [incremental clustering, dual-threshold constraint, memory-efficient, lightweight transformer, cache management]
  ...
  - **link:** https://arxiv.org/pdf/2512.24711
  ...
  - **thumbnail:** https://pub-9ba4a5dae3bf4fc7b7d26411a74e92db.r2.dev/thumbnails/34337ad38d20be1dae6a4c4cc881ccab3fb447fa75e8bee3e7a838015b008cbb_w640_q70.webp
  - **Simple LLM Summary:** The paper addresses the challenge of balancing efficiency and performance in incremental clustering for long-text coreference resolution. It proposes MEIC-DT, a memory-efficient method featuring a dual-threshold constraint mechanism, a statistics-aware eviction strategy, and an internal regularization policy. Experiments show MEIC-DT achieves competitive performance under strict memory constraints.
  - **Mindmap:**

    ```mermaid
    graph TB
        A[MEIC-DT: Memory-Efficient Incremental Clustering for Long-Text Coreference Resolution with Dual-Threshold Constraints] --> B[核心问题/Problem: Balancing efficiency and performance in incremental clustering for long texts]
        A --> C[主要方法/Method: Dual-threshold constraint, Statistics-Aware Eviction Strategy (SAES), Internal Regularization Policy (IRP)]
        A --> D[关键结果/Results: Achieves highly competitive coreference performance under stringent memory constraints]
    ```

- **[arXiv260101] MDiffFR: Modality-Guided Diffusion Generation for Cold-start Items in Federated Recommendation**
  - **tags:** [mlsys], [federated learning], [federated recommendation, item cold-start, diffusion model, modality-guided generation, privacy guarantee]
  - **authors:** Kang Fu, Honglei Zhang, Xuechao Zou, Yidong Li
  - **institution:** Beijing Jiaotong University, Key Laboratory of Big Data & Artificial Intelligence in Transportation, Ministry of Education, China
  - **link:** https://arxiv.org/pdf/2512.24715
  ...
  - **thumbnail:** https://pub-9ba4a5dae3bf4fc7b7d26411a74e92db.r2.dev/thumbnails/0043abb7859816bbc8cd02d819ea36ea1dcb659896ddccf82fe3a147387a46e8_w640_q70.webp
  - **Simple LLM Summary:** This paper addresses the challenging item cold-start problem in Federated Recommendation (FR) by proposing MDiffFR, a method that uses a modality-guided diffusion model on the server to generate item embeddings. The generated embeddings are distributed to clients for inference, leveraging modality features to guide the generation process and improve alignment. Experiments on four real datasets show that MDiffFR outperforms existing baselines and provides stronger privacy guarantees.
  - **Mindmap:**

    ```mermaid
    graph TB
        A[MDiffFR: Modality-Guided Diffusion Generation for Cold-start Items in Federated Recommendation] --> B(核心问题/Problem)
        A --> C(主要方法/Method)
        A --> D(关键结果/Results)
        B --> B1[联邦推荐中的项目冷启动问题 / Item Cold-Start in Federated Recommendation]
        B --> B2[严格隐私限制导致表示学习困难 / Strict Privacy Constraints Hinder Representation Learning]
        C --> C1[服务器端使用定制扩散模型生成嵌入 / Use Tailored Diffusion Model on Server to Generate Embeddings]
        C --> C2[预训练模态编码器提供条件信号 / Pre-trained Modality Encoder Provides Conditional Signals]
        D --> D1[在四个真实数据集上超越基线 / Outperforms Baselines on Four Real Datasets]
        D --> D2[提供更强的隐私保证 / Provides Stronger Privacy Guarantees]
    ```

- **[arXiv260101] OpenOneRec Technical Report**
  - **tags:** [mlsys], [llm training], [generative recommendation, foundation model, co-pretraining, RecIF-Bench, catastrophic forgetting]
  ...
  - **link:** https://arxiv.org/pdf/2512.24762
  - **code:** https://github.com/Kuaishou-OneRec/OpenOneRec
  - **contributions:** 1) Proposed RecIF-Bench, a holistic benchmark with 8 tasks for evaluating integrated recommendation capabilities, and released a large open training dataset. 2) Open-sourced a comprehensive training pipeline (data processing, co-pretraining, post-training) to ensure reproducibility and demonstrated predictable scaling of recommendation capabilities while mitigating catastrophic forgetting. 3) Released the OneRec-Foundation model family (1.7B and 8B), achieving new SOTA on RecIF-Bench and showing strong cross-domain transferability with a 26.8% average improvement on the Amazon benchmark.
  - **thumbnail:** https://pub-9ba4a5dae3bf4fc7b7d26411a74e92db.r2.dev/thumbnails/479a2e2d3cfe6c5155f120df18d59b7783da83d3d0ced554b30ba633be6d5b8c_w640_q70.webp
  - **Simple LLM Summary:** This paper addresses the gap between current generative recommendation models and general intelligence by introducing an open foundation model and benchmark. The proposed method involves co-pretraining and post-training a large model on a massive dataset, resulting in the OneRec-Foundation models which achieve state-of-the-art performance on recommendation tasks while retaining general knowledge, demonstrating a step towards more intelligent recommender systems.
  - **Mindmap:**

    ```mermaid
    graph TB
        A[OpenOneRec Technical Report] --> B[核心问题/Problem]
        A --> C[主要方法/Method]
        A --> D[关键结果/Results]
        B --> B1[推荐系统与通用智能存在差距/Gap between RecSys and General Intelligence]
        B --> B2[缺乏综合评估基准/Lack of Holistic Benchmark]
        C --> C1[提出RecIF-Bench与开源数据/Propose RecIF-Bench & Open Data]
        C --> C2[开源完整训练框架/Open-source Full Training Pipeline]
        C --> C3[发布OneRec-Foundation模型/Release OneRec-Foundation Models]
        D --> D1[在RecIF-Bench上实现SOTA/Achieve SOTA on RecIF-Bench]
        D --> D2[在Amazon基准上显著提升/Substantial Improvement on Amazon Benchmark]
        D --> D3[缓解灾难性遗忘/Mitigate Catastrophic Forgetting]
    ```
